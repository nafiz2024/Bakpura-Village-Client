...
const publicRoutes = [
  ['', 'HOME — Coming in Client Step 02'], ['about', 'আমাদের সম্পর্কে'], ['activities', 'কার্যক্রম'],
...
export default function AppRoutes() {
  return <><ScrollToTop/><Routes><Route element={<PublicLayout/>}>{publicRoutes.map(([path, title]) => <Route key={path} index={path === ''} path={path || undefined} element={<PagePlaceholder title={title}/>}/>)}</Route><Route path="/admin/login" element={<PagePlaceholder title="Admin Login — পরবর্তী ধাপে তৈরি হবে"/>}/><Route path="/admin" element={<AdminLayout/>}><Route index element={<Navigate to="dashboard" replace/>}/>{adminRoutes.map(([path, title]) => <Route key={path} path={path} element={<PagePlaceholder title={title}/>}/>)}</Route><Route path="*" element={<NotFoundPage/>}/></Routes></>
}
