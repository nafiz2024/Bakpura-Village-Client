...
const isPublic = item => item?.archived !== true && item?.isArchived !== true && (!item.status || item.status === 'published' || item.isPublished === true)
export default function ActivitiesPage(){
  ...
  return <><ActivitiesHero banner={settings.branding?.banner}/><ActivityFilters category={category} search={search} sort={sort} onCategory={value=>change({category:value})} onSearch={value=>change({search:value})} onSort={value=>change({sort:value})}/><FeaturedActivity item={featured}/><ActivitiesGrid items={state.items} loading={state.key!==requestKey} error={state.error} onRetry={()=>setReload(x=>x+1)} onReset={reset}/><Pagination page={state.meta.page} pages={state.meta.pages} onPage={value=>update({page:value})}/><ImpactAndProcess/><Collaboration/><UpcomingActivities items={state.items}/><ActivityGallery items={state.items}/><TransparencyAndCTA activities={state.items} settings={settings}/></>
}
